...
df_expo <- read_exposure_data('eqtls_bag/significant_brain_eqtls.csv',
                              sep = ',',
...
for (i in 1:length(genes)) {
  ...
  for (j in 1:length(phenos)) {
    ...
    if (pheno_gwas_id[j, 3] == 0) {
      ...
      try_times <- 0
      while (T) {
        try(pheno <- extract_outcome_data(
        ...
        if (try_times >= 20) {
          print('Tried too many times, maybe there is no network connnection
                or the open gwas server is currently unreachable. Abort.')
          break
        }
      }
    ...
    if (is.null(pheno)) {
      sprintf('No snp matches for exposure %s and outcome %s',
              genes[i], phenos[j])
      next
    }
    ...
    if ((nrow(dat) == 0) || nrow(dat[dat$mr_keep == T, ] ) == 0) {
      ...
      write.csv(res_all, 'results_bag/phwas_res_brain.csv', row.names = F)
      write.csv(res_pleiotropy, 'results_bag/pleiotropy_res_brain.csv', row.names = F)
    }
  }
}
