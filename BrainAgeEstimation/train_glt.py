...
def main(args):
    ...
    sys.stdout = Logger(filename=os.path.join(cwd, 'logs', 'train_glt.log'))

    data_path = os.path.join(cwd, 'data')
    image_path = os.path.join(data_path, 'npy_data')
    save_path = os.path.join(cwd, 'saved_models', 'glt')
    res_path = os.path.join(cwd, 'results', 'glt')
    ...
    for tr_idx, va_idx in kf.split(x):
        ...
        if os.path.exists(os.path.join(save_path, f'model_vit_fold{tr_fold}.pth')):
            ...
            model = GLT3D(1, patch_size=args.patch_size, step=args.step,
                          drop_rate=args.dropout, backbone='vgg8')
            ...
            train_and_evaluate(args, device, model, optimizer, tr_loader, va_loader, tr_fold=tr_fold,
                               save_path=save_path, save_name=f'model_vit_fold{tr_fold}', lr_scheduler=lr_scheduler)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bh_size', type=int, default=4)
