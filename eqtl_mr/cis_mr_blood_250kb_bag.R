...
df <- read.csv('data_250kb_eqtls/druggable_blood_eqtls_new.csv')
print(length(unique(df$ensembl_gene_id)))

df_expo <- read_exposure_data('data_250kb_eqtls/druggable_blood_eqtls.csv',
                              sep = ',',
...
for (i in 1:length(genes)) {
  ...
  expo_clamped$eaf.exposure <- 1 - expo_clamped$eaf.exposure
  ...
  if ((nrow(dat) == 0) || nrow(dat[dat$mr_keep == T, ] ) == 0) {
    ...
    write.csv(res_all, 'results_250kb_eqtls_bag/mr_res_blood.csv', row.names = F)
    write.csv(res_pleiotropy, 'results_250kb_eqtls_bag/pleiotropy_res_blood.csv', row.names = F)
  }
}
