...
from model.sfcn.sfcn import SFCN
...
from train_and_eval_sfcn import train_and_evaluate, evaluate2
...
def main(args):
    ...
    sys.stdout = Logger(filename=os.path.join(cwd, 'train_sfcn.log'))

    data_path = os.path.join(cwd, 'data')
    image_path = os.path.join(data_path, 'npy_data')
    save_path = os.path.join(cwd, 'saved_models', 'sfcn')
    res_path = os.path.join(cwd, 'results', 'sfcn')
