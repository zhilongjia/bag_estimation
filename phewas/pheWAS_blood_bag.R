...
df_expo <- read_exposure_data('eqtls_bag/significant_blood_eqtls.csv',
                              sep = ',',
...
mr_res_path <- sprintf('results_bag/phwas_res_blood.csv') 
pleiotropy_res_path <- sprintf('results_bag/pleiotropy_res_blood.csv')
...
for (i in 1:length(genes)) {
  ...
  for (j in 1:length(phenos)) {
    ...
    x <- subset.data.frame(res_all, id.exposure == genes[i] & id.outcome == phenos[j])
    if (nrow(x) > 0) {
      print(sprintf('Done. Analying %s on %s', genes[i], phenos[j]))
      next
    }
    ...
    if ((nrow(dat) == 0) || nrow(dat[dat$mr_keep == T, ] ) == 0) {
      ...
      write.csv(res_all, 'results_bag/phwas_res_blood.csv', row.names = F)
      write.csv(res_pleiotropy, 'results_bag/pleiotropy_res_blood.csv', row.names = F)
    }
  }
}
