...
df <- read.csv('pqtl_data/pqtls_interval_bag.csv')
print(length(unique(df$id)))


df_expo <- read_exposure_data('pqtl_data/pqtls_interval_bag.csv',
                              sep = ',',
                              phenotype_col = 'id',
                              snp_col = 'SNP',
                              beta_col = 'Effect',
...
mr_res_path <- sprintf('results_interval_bag/mr_res_blood.csv') 
pleiotropy_res_path <- sprintf('results_interval_bag/pleiotropy_res_blood.csv')
...
for (i in 1:length(genes)) {
  ...
  if ((nrow(dat) == 0) || nrow(dat[dat$mr_keep == T, ] ) == 0) {
    ...
    write.csv(res_all, 'results_interval_bag/mr_res_blood.csv', row.names = F)
    write.csv(res_pleiotropy, 'results_interval_bag/pleiotropy_res_blood.csv', row.names = F)
  }
}
